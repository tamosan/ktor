...
class DeflaterReadChannelTest {
    ...
    fun testWithRealFile() {
        ...
        file.readChannel().use {
            testReadChannel(file.readText(), it.toByteReadChannel())
        }

        file.readChannel().use {
            testWriteChannel(file.readText(), it.toByteReadChannel())
        }
    }
    ...
    private fun testWriteChannel(expected: String, src: ByteReadChannel) {
        ...
        val result = runBlocking {
            channel.toByteArray().inputStream().ungzip().reader().readText()
        }
